...
def load_fda_meanbrain(fixed_path=None):
...
def warp_STA_brain(STA_brain, fly, fixed, anat_to_mean_type, dataset_path=None):
    n_tp = STA_brain.shape[1]
    ...
    moving_resolution = (2.611, 2.611, 5)  # THIS SHOULD NOT BE HARDCODED
    # ##########################
    # ## Organize Transforms ###
    # ##########################
    warp_directory = os.path.join(dataset_path, fly, "registration/transforms")
    ...
    func_to_anat_warp_file = os.path.join(warp_directory, "func_to_anat_1Warp.nii.gz")
