...
from brainsss2.visual import load_photodiode, get_stimulus_metadata, extract_stim_times_from_pd
...
def parse_args(input, allow_unknown=True):
    ...
    parser.add_argument('--atlasdir', type=str,
        help='atlas directory', required=True)
    parser.add_argument('--atlasfile', type=str,
        default='20220301_luke_2_jfrc_affine_zflip_2umiso.nii')
    parser.add_argument('--roifile', type=str,
        default='jfrc_2018_rois_improve_reorient_transformed.nii')
    parser.add_argument('--explosionroifile', type=str,
...
def plot_stb():
    ...
    for angle,color in zip([0,180],['blue','red']):
        ...
        plt.fill_between(np.arange(len(mean_trace[angle])),
...
def old_STA():
    ...
    for condition in ['ve_no_0','ve_no_180','ve_0','ve_180']:
        ...
        tempfilt_STA_brain = gaussian_filter1d(reformed_STA_brain,sigma=1,axis=1,truncate=1)
...
def get_resampled_slice_data(all_signals, event_times_list, slice,
                             scale_data=True, interpolation_type='linear'):
    ...
    all_signals_slice_df = pd.DataFrame(slicedata,
        index=pd.to_datetime(timestamps[:, 0]/1000, unit='s'))

    resampled_df = all_signals_slice_df.resample('100ms').mean().interpolate(
        method=interpolation_type)

    event_times = pd.to_datetime(np.array(event_times_list) / 1000, unit='s')
    event_df = pd.DataFrame({'event': np.zeros(resampled_df.shape[0])},
                            index=resampled_df.index)
    event_df = pd.concat((event_df, pd.DataFrame({'event': np.ones(event_times.shape[0])},
                            index=event_times)))
    event_df_resampled = event_df.resample('100ms', origin=all_signals_slice_df.index[0]).sum()
...
def run_fir_regression(resampled_df, desmtx_df, zero_onset=True):
    ...
    coefs = lr.coef_
    if zero_onset:
        coefs = coefs - coefs[np.where(desmtx_df.columns==0.)[0][0]]
...
def supervoxels_to_img(supervoxel_data, cluster_img):
    ...
    output_img = nib.Nifti1Image(np.zeros(output_shape), cluster_img.affine,
                                    cluster_img.header)
    ...
    for slice in range(supervoxel_data.shape[0]):
        ...
        for timepoint in range(supervoxel_data.shape[-1]):
            for cluster in np.unique(slicemask):
                slicedata[slicemask == cluster] = supervoxel_data[slice, cluster, timepoint]
...
if __name__ == "__main__":
    ...
    behavior_traces, mean_trace, sem_trace = extract_STB(
        fictrac, starts_angle)
    ...
    for condition in ['ve_no_0','ve_no_180','ve_0','ve_180']:
        ...
        for slice in range(all_signals.shape[0]):
            ...
            if all_coefs is None:
                all_coefs = np.zeros((all_signals.shape[0], coefs.shape[0], coefs.shape[1]))
        ...
        img.to_filename(os.path.join(
            STA_path, f'STA_coefs_{condition}.nii.gz'))
        img = supervoxels_to_img(all_rsquared, cluster_label_img)
        img.to_filename(os.path.join(
            STA_path, f'STA_rsquared_{condition}.nii.gz'))
    outfile.close()
