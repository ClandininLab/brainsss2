...
def parse_args(input, allow_unknown=True):
    ...
    parser.add_argument('--nclusters', type=int, default=2000,
        help='number of clusters to use for supervoxelization')
    parser.add_argument(
        '-d', '--dir',
        type=str,
        help='func dir to be processed',
        required=True)
...
if __name__ == "__main__":
    ...
    if os.path.exists(cluster_dir) and args.overwrite:
        ...
        raise FileExistsError(f"cluster dir {cluster_dir} already exists, use -o to overwrite")
    ...
    cluster_labels = []
    cluster_img = nib.Nifti1Image(np.zeros(brain.shape[:-1]), brain.affine, brain.header)
