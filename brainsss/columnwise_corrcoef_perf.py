...
def AlmightyCorrcoefNaive(O, P, C):
    ...
    DO = O - (
        np.sum(O, 0) / np.double(n)
    )  # compute O - mean(O); note that mean(O) will be appleid row-wise to O
...
def AlmightyCorrcoefEinsumOptimized(O, P, verbose=False):
    (n, t) = O.shape  # n traces of t samples
    (n_bis, m) = P.shape  # n predictions for each of m candidates
    if verbose:
        print(f"n = {n}, t = {t}")
        print(f"n_bis = {n_bis}, m = {m}")
...
"""
    ...
    print(
        min(
            timeit.repeat(
                "AlmightyCorrcoefEinsumOptimized(O, P)",
                setup=timingSetup,
                repeat=3,
                number=1,
            )
